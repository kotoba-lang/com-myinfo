(ns myinfo.main
  "Kotodama WASM entrypoint for the myinfo clean-room actor (L4) — Clojure port.
  ...
  facts (`myinfo.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "myinfo")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"myinfo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "myinfo-compat" :tier tier :entities entities} 200])
...
(defn start! [] :myinfo-compat/ready)
